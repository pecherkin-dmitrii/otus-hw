...
var textEng = ` Formatting issues are the most contentious but the
	least consequential. People can adapt to different formatting styles
	but it's better if they don't have to, and less time is devoted to
	the topic if everyone adheres to the same style. The problem is how
...
var textSmall = "  Formatting   issues   are  the  most contentious  but the"
...
func TestTop10(t *testing.T) {
	...
	t.Run("positive test with English text", func(t *testing.T) {
		expected := []string{
			"the",      // 8
			"to",       // 6
			"and",      // 5
	...
	t.Run("positive test with small text", func(t *testing.T) {
		expected := []string{
			"the",         // 2
			"Formatting",  // 1
			"are",         // 1
		...
		require.Equal(t, expected, Top10(textSmall))
	})
}
